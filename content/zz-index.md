---
title: Index
---

# lostineconomics, v4

**My world is a sort of LIE, Lost In Economics.**

- 허준석의 블로그입니다.
- Obsidian과 연동해서 쓰고 있어요. 디자인은 다소 아쉽습니다. 텍스트를 사랑하는 제 입장에서는 오히려 이 편이 좋습니다. 
- 경제학 전공한 아재 예비 은퇴자입니다. 개인적인 연락은 [여기](mailto:anarinsk@gmail.com)로 주세요.
- 메뉴 (의미가 있는지 모르겠으나...)
	- **개발잡기**: 이 블로그를 포함한 개인적인 '개발' 관련 
	- **독서잡기**: 읽은 책들, 추천할 책들 
	- **사유잡기**: 생각할 거리들 
	- **시험잡기**: 블로그를 위한 여러 테스트 용도의 포스트
	- **신변잡기**: 
	- **잡기잡기**: 무작위로 떠오른 무엇 중 붙잡거나 기억하고 싶은 것들 
	- **투자잡기**: 투자 관련 

## History of LIE

- [v4](https://blog.lostineconomics.com/) built with [Quartz](https://quartz.jzhao.xyz/) and [Github Pages](https://pages.github.com/)
- [v3](https://anarinsk.github.io/lostineconomics_quarto/) built with [Quarto](https://quarto.org/) and [Github Pages](https://pages.github.com/)
- [v2](https://anarinsk.github.io/lostineconomics-v2-1/) built with [fastpages](https://github.com/fastai/fastpages) and [Github Pages](https://pages.github.com/)
- [v1](https://anarinsk.github.io/lostineconomics-v1/) built with markdown and [Github](https://www.github.com/)
- [v0](https://lostineconomics.netlify.com/) built with [BlogDown](https://bookdown.org/yihui/blogdown) and [Github Pages](https://pages.github.com/)

## Side Projects 

###  [SHOW of anari](https://show.lostineconomics.com/)

- 본 영화와 드라마를 기록하는 공간 
- 대단한 무엇보다는 무엇을 봤는지를 기록하는 곳 
- 매년 조촐하게 하는 연말 결산 때 최근에 본 것으로 쏠리는 바이어스를 줄이기 위한 개인적인 안전 장치
